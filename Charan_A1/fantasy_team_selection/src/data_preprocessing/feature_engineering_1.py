...
def main(input_file_path, output_dir, k, output_file_name, num_threads):
    ...
    pvp_df = pd.read_csv(os.path.join(current_dir, "..", "data", "interim", "ipl","T20_pvp.csv"))
    ...
    new_df["is_impace_player_rule"] =  np.where (new_df["date"] >= pd.to_datetime("2023-02-02"),1,0)
