...
if len(sys.argv) != 4:
    print("Usage: python3 data_processing.py <file_path> <output_dir>")
...
def generate_df(file_path):
    ...
    generalized_format = match_format_generalised[match_format]
    df["format"] = generalized_format
    df["match_id"] = file_path.split("/")[-1].split(".")[0]

    df = calculate_fantasy_points(df, generalized_format)
    return df
...
def data_info_extraction(readme_path):
    ...
    cutoff_date = pd.to_datetime("2024-11-10")
...
info_df = data_info_extraction(f"{json_data_dir}/README.txt")
...
df_t20 = grouped.get_group('T20')
df_od = grouped.get_group('OD')

df_test.to_csv(f"{output_dir}/Test.csv", index=False)
df_od.to_csv(f"{output_dir}/OD.csv", index=False)
df_t20.to_csv(f"{output_dir}/T20.csv", index=False)
