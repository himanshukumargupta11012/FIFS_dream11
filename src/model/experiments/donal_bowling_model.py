...
from donal_columns import bowling_cols
...
MODEL_SAVE_PATH = "donal_saved_models/best_bowling_model.pkl"
TRAIN_START_DATE = pd.Timestamp("2010-01-01")
FEATURES_SAVE_PATH = "features/selected_bowling_features.pkl"
...
def main(evaluate_on_test=EVALUATE_ON_TEST, scale_features=False):
    ...
    target_column = "bowling_fantasy_points"
    categorical_cols = ['player', 'team', 'opposition', 'date', 'venue', 'match_id', 'player_id']
    numerical_features = [col for col in bowling_cols if col not in categorical_cols and col != target_column]
...
if __name__ == "__main__":
    main(evaluate_on_test=EVALUATE_ON_TEST, scale_features=SCALE_FEATURES)
