...
# function for predicting the scores and generating explanations
def forward_charan(date, players_df, venue, toss_result, k, device,weight_mlp,weight_gbm):

    interim_csv = f"/home/ai21btech11012/FIFS_dream11/src/data_charan/processed/combined/10_IPL.csv"

    interim_df = pd.read_csv(interim_csv)
    interim_df["date"] = pd.to_datetime(interim_df["date"])
    processed_final = interim_df

    # print("Venue ",interim_df[(interim_df["date"] == date)]["venue"])
    # print(venue)
    processed_final = processed_final[
         (processed_final["date"] == date) & (
        (processed_final["venue"] == venue) |
        (processed_final["venue"].str.contains(venue, case=False, na=False)) |
        (processed_final["venue"].apply(lambda x: venue in x if isinstance(x, str) else False)) |
        (processed_final["venue"].apply(lambda x: x in venue if isinstance(x, str) else False))
    )
    ]
    ...
    # print("Before : ",players_df)
    # print("Before : ",processed_final)
    players_df = players_df[players_df["identifier"].isin(processed_final["player_id"])]
    processed_final = processed_final[processed_final["player_id"].isin(players_df["identifier"])]

    # print(players_df)
    # print(processed_final)
    args = Namespace(mlp_model_path=f"{current_dir}/../mlp_fixed_output_1/mlp_k{k}_final_model.pth", lgbm_model_path=f"{current_dir}/../lgbm_fixed_output_2/lgbm_k{k}_final_model.joblib", scalers_path=f"{current_dir}/../mlp_fixed_output_1/mlp_k{k}_scalers.pkl", feature_names_path=f"{current_dir}/../lgbm_fixed_output_2/lgbm_k{k}_feature_names.pkl", k=k, device=device, mlp_weight=weight_mlp, lgbm_weight=weight_gbm)
    output = load_and_test_model(args, processed_final, device)
    ...
    players_df["prev_fantasy_points"] = processed_final[f"fantasy_points_ewma_10"].values

    team = select_team(None, players_df, "prev_fantasy_points")

    # top_players = team.sort_values(by='prev_fantasy_points', ascending=False).head(2)
    top_players = team.sample(n = 2)
    ...
    team["C/VC"] = output_df["C/VC"]
    total_poi = 0.0

    for _,row in team.iterrows():
        player_row = processed_final[processed_final["player_id"] == row["identifier"]]

        fantasy_points = player_row["fantasy_points"].item()
        if row["C/VC"] == "C" :
            fantasy_points = max(0,fantasy_points*2.0)
        if row["C/VC"] == "VC" :
            fantasy_points = max(0,fantasy_points*1.5)
        total_poi += fantasy_points
