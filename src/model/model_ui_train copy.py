...
from feature_utils import process, normalise_data
import numpy as np
import argparse
import pickle
import os
...
def train_and_test_model(train_start_date, train_end_date, test_start_date, test_end_date, output_csv):
    ...
    for format in ["OD", "T20", "Test"]:
        df = pd.read_csv(f"{current_dir}/../data/processed/combined/{k}_{format}.csv")
        ...
        # if train_df.empty:
        #     print(f"No training data found for {format} format")
        ...
        scalers_dict[f"{format}_x"] = scaler_X
        scalers_dict[f"{format}_y"] = scaler_y
        ...
        pred = test_model(model, test_loader, device = device).view(-1, 1)
        pred = scalers_dict[f"{format}_y"].inverse_transform(pred).squeeze(1).astype(int)
    ...
    with open(f'{current_dir}/../model_artifacts/model_{train_end_date}_{format}_scalers.pkl', 'wb') as file:
...
if __name__ == "__main__":
    train_and_test_model("2022-11-01", "2024-11-10", "2024-11-10", "2024-11-15", "src/data/processed/model_ui_output_2024-11-15.csv")