...
from predict_utils_copy import get_toss_result, forward_himanshu, forward_charan, teams_shortform, get_players_info, get_shortform
...
ipl_matches_info = pd.read_csv(f"/home/ai21btech11012/FIFS_dream11/src/testing_data/ipl_2025_schedule_2_new.csv")
...
if __name__ == "__main__":
    ...
    # date = today
    # date = "2025-04-19"
    files = os.listdir("/home/ai21btech11012/FIFS_dream11/src/data/processed/back_test/")

    weights_mlp = np.linspace(0,1,3)
    result_dict = {}

    for weight_mlp in weights_mlp :
        weight_lgbm = 1 - weight_mlp
        
        sorted_files = sorted(files, key=lambda s: int(s.split('_')[2][:-4]))
        final_points = []
        i = 0
        for file in sorted_files :
            ...
            if file  is not None:
                ...
                players_df = pd.read_csv("/home/ai21btech11012/FIFS_dream11/src/data/processed/back_test/" + file)
                ...
                for _, row in match_infos.iterrows():
                    if row["Team 1"] == team1_cricbuzz or row["Team 2"] == team1_cricbuzz:
                        match_info = row
                        break
            ...
            if args.ensemble:
                f = forward_charan(date, players_df, venue, toss_result, k, device,weight_mlp,weight_lgbm)
                final_points.append(f)
            else:
                # loading the model
        ...
        result_dict[(weight_mlp,weight_lgbm)] = np.mean(final_points)
   
    sorted_dict = dict(sorted(result_dict.items(), key=lambda item: item[1], reverse=True))

    import csv
    name_file = "new_lgbm"
    with open(f"grid_search_{name_file}.csv", mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["weight_mlp", "weight_lgbm", "fantasy_mean"])  # header
        for (k1, k2), value in sorted_dict.items():
            writer.writerow([float(k1), float(k2), value])

        
# python predict_model\ copy.py --k 10 --ensemble True