...
def MLP_train(k, dim, batch_size, args):
    scalers_dict = {}
    format = ["Test","OD","T20"]

    for game_format in format:

        print(f" -------------------------------- {game_format} ---------------------------------------")
        train_data_path = os.path.join("..", "data", "processed", "train", f"{k}_{game_format}.csv")
        test_data_path = os.path.join("..", "data", "processed", "test", f"{k}_{game_format}.csv")
        ...
        scalers_dict[f"{game_format}_x"] = scaler_X
        scalers_dict[f"{game_format}_y"] = scaler_y
        ...
        model = train_model(full_model, train_loader, test_loader, args, game_format, should_save_best_model=False,device=device)
        torch.save(model.state_dict(), f"../model_artifacts/{game_format}_model.pth")
    ...
    with open(f'../model_artifacts/model_scalers.pkl', 'wb') as file:
...
def main():
    ...
    MLP_train(k, dim, batch_size, args)
